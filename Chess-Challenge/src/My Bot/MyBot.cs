...
// 1016/1024 tokens
// (Additional helper functions and comments stripped for submission)
            ...
            for (var bitmapIdx = 0; bitmapIdx < 78; bitmapIdx++)
            {
                ...
                for (_i = 0; _i < 64; _i++)
                    dynamicData[bitmapIdx / 6 * 64 + _i] +=
                        ChessChallenge.Chess.BitBoardUtility.ContainsSquare(initData[bitmapIdx], _i) ? 1 << (bitmapIdx % 6) : 0;
            }
        }
